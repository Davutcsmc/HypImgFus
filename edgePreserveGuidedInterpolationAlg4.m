function [refImageInterpolated] = edgePreserveGuidedInterpolationAlg4(guideImage,refImage,distPower)
...
for i=2:3:wm-1
    for j=2:3:hm-2
        ...
        %%% kenarbölgesi
        if ((i-3)<=0) || ((i+3)>wm)
            nPxls = guideImage(i,j+1:j+2);
            cPxls = guideImage(i,j:3:j+3);
            ...
            simMatris = gSimN.*(distValsEdgeRegion.^distPower);
            ...
            nPxls = guideImage(i,j+1:j+2);
            cPxls = guideImage(i-3:3:i+3,j:3:j+3);
            
            gDiff = abs( repmat(nPxls(:)',[6,1]) - repmat(cPxls(:),[1,2]) );
            gDiffN = gDiff./repmat(sum(gDiff,1)+eps,[6,1]);
            
            gSim = 1./(gDiffN + repmat(min(gDiffN+(gDiffN==0)),[6,1])*0.1);
            gSimN = gSim./repmat(sum(gSim,1),[6,1]);
            
            simMatris = gSimN.*(distValsMiddleRegion.^distPower);
...
for i=2:3:wm-2
    for j=2:3:hm-1
        ...
        %%% kenarbölgesi
        if ((j-3)<=0) || ((j+3)>hm)
            nPxls = guideImage(i+1:i+2,j);
            cPxls = guideImage(i:3:i+3,j);
            ...
            nPxls = guideImage(i+1:i+2,j);
            cPxls = guideImage(i:3:i+3,j-3:3:j+3);
...
for i=3:3:wm-3
    for j=3:3:hm-3
        ...
        nPxls = guideImage(i:i+1,j:j+1);
        cPxls = guideImage(i-1:3:i+2,j-1:3:j+2);
