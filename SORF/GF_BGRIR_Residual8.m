function [ I_HS_BGRIR ] = GF_BGRIR_Residual8( I_HS,I_PAN, dataset, distPower )
...
for m=1:1:wh
    for n=1:1:hh
        ...
        local_HS = I_HS(minWh:maxWh,minHh:maxHh,:);
        local_MS = I_PANds(minWh:maxWh,minHh:maxHh,:);
        
        for i=1:1:dh
            bandH = local_HS(:,:,i);
            for j=1:1:dm
                bandP = local_MS(:,:,j);
                ccVals(m,n,i,j) = corr2(bandH,bandP);
            end
        end
    end
end
...
for m=1:1:wh
    for n=1:1:hh
        ccValsLocal = squeeze(ccVals(m,n,:,:));
        [~,ccInds] = max(ccValsLocal,[],4);
        for i=1:1:dh
            if ccInds(i) == 1
                I_HS_BGRIR((m-1)*3+1:m*3,(n-1)*3+1:n*3,i) = I_HS_BE((m-1)*3+1:m*3,(n-1)*3+1:n*3).*sRatioB_EdgePreserveInterpolation((m-1)*3+1:m*3,(n-1)*3+1:n*3,i);
            elseif ccInds(i) == 2
                I_HS_BGRIR((m-1)*3+1:m*3,(n-1)*3+1:n*3,i) = I_HS_GE((m-1)*3+1:m*3,(n-1)*3+1:n*3).*sRatioG_EdgePreserveInterpolation((m-1)*3+1:m*3,(n-1)*3+1:n*3,i);
            elseif ccInds(i) == 3
                I_HS_BGRIR((m-1)*3+1:m*3,(n-1)*3+1:n*3,i) = I_HS_RE((m-1)*3+1:m*3,(n-1)*3+1:n*3).*sRatioR_EdgePreserveInterpolation((m-1)*3+1:m*3,(n-1)*3+1:n*3,i);
            else
                I_HS_BGRIR((m-1)*3+1:m*3,(n-1)*3+1:n*3,i) = I_HS_IRE((m-1)*3+1:m*3,(n-1)*3+1:n*3).*sRatioIR_EdgePreserveInterpolation((m-1)*3+1:m*3,(n-1)*3+1:n*3,i);
            end
        end
