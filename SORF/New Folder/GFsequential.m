function [ I_HS_seq ] = GFsequential( I_HS,I_PAN, dataset )
...
%   Detailed explanation goes here
    ...
    [wh,hh,dh] = size(I_HS);
    sRatio = I_HS(:,:,1:end-1) ./( I_HS(:,:,2:end) +1e-2);
...
%     subplot(144), imshow( I_HS_IRE,[] );
    ...
    I_HS_seq(:,:,maxInds(1)) = I_HS_BE;
    
    bantInit = I_HS_BE;
    for i=maxInds(1)-1:-1:1
        bantInit = bantInit.*sRatio_cubic(:,:,i);
        I_HS_seq(:,:,i) = bantInit;
    end
       
    bantInit = I_HS_BE;
    for i=maxInds(1):1:size(sRatio,3)
        bantInit = bantInit./sRatio_cubic(:,:,i);
        I_HS_seq(:,:,i+1) = bantInit;
    end 
...
%          plot(squeeze(I_REF(xpos,ypos,:)),'b','LineWidth',2);
